...
@Injectable()
export class OrganizationAssetStorageService {
  constructor(private readonly configService: ConfigService) {}

  async upload(input: OrganizationAssetUpload): Promise<string> {
    ...
    if (!bucket || !accessKeyId || !secretAccessKey) {
      return `s3://local-organization-assets/${key}`;
    }
  ...
  private buildObjectKey(input: OrganizationAssetUpload): string {
