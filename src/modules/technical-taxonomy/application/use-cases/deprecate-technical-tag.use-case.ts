...
@Injectable()
export class DeprecateTechnicalTagUseCase {
  constructor(@Inject(TECHNICAL_TAG_REPOSITORY) private readonly repository: TechnicalTagRepository) {}

  async execute(input: { organizationId: string; id: string; deprecatedBy: string }): Promise<Result<TechnicalTagResponse, Error>> {
    try {
      ...
      tag.deprecate(input.deprecatedBy);
